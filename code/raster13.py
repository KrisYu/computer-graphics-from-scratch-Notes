    ...
    for light in lights:
        if light.ltype == 'AMBIENT':
            ...
            # only direction will effect directional light
                transformed_directional_light = camera.orientation.get_inverse_rotation().transform_vec3(light.position)
                vec_l = -transformed_directional_light

            n_dot_l = normal.dot(vec_l)
            if n_dot_l > 0:
                illumination += light.intensity * n_dot_l / (normal.length * vec_l.length)
            ...
            if r_dot_v > 0:
                specular = 50
                illumination += light.intensity * pow((r_dot_v / (view.length * vec_r.length)), specular)
